...
func mysql(host, port, database, username, password, folderPath, action, statefile, prescript, postscript, difference, tag string, logger *log.Logger) {
	dbName = database
	switch action {
	case "migrate":
		mysql_calling(host, port, database, username, password, folderPath, tag, logger)
	case "clear":
		mysqlcleardata(host, port, database, username, password, logger)
	case "clearall":
		mysqlclearAllData(host, port, database, username, password, logger)
	case "check":
		mysqlcheckScripts(host, port, database, username, password, folderPath, logger)
	case "snapshot":
		mysqlGenerateMigrationScript(host, port, database, username, password, folderPath, statefile, logger)
	case "delta":
		mysql_processFiles(folderPath, prescript, postscript, difference)
	default:
		logger.Println("Unsupported action:", action)
	}
}

func mysql_calling(host, port, database, username, password, folderPath, tag string, logger *log.Logger) {
	// Establish a database connection
	db, err := sql.Open("mysql", fmt.Sprintf("%s:%s@tcp(%s:%s)/%s", username, password, host, port, database))
	if err != nil {
		logger.Fatalf("Error connecting to MySQL database: %v", err)
	}
	defer db.Close()
	...
	hashCodes := mysqlRetrieveHashCodes(db)
	...
	err = filepath.Walk(folderPath, func(path string, info os.FileInfo, err error) error {
		...
		versions, fileName, hashKeys, scripts, tags := mysqlGetVersion(path)
		...
		for i, version := range versions {
			...
			if mysqlContains(hashCodes, hk) && !strings.HasPrefix(fileName, "c") {
				status = "skipped"
				...
				sqlScript := mysqlRemoveComments(script)
			...
			existingRecordID := mysqlCheckExistingRecord(db, majorValue, minorValue, version)
...
func mysqlGetTableNames(db *sql.DB) ([]string, error) {
...
func mysqlGenerateScriptForTable(db *sql.DB, tableName string) (string, error) {
...
func mysqlGenerateMigrationScript(host, port, database, username, password, folderPath, statefile string, logger *log.Logger) {
	...
	tableNames, err := mysqlGetTableNames(db)
	if err != nil {
	...
	for _, tableName := range tableNames {
		script, err := mysqlGenerateScriptForTable(db, tableName)
		if err != nil {
...
func mysqlRetrieveHashCodes(db *sql.DB) []string {
...
func mysqlRemoveComments(sqlScript string) string {
...
func mysqlGetVersion(filePath string) ([]string, string, []string, []string, []string) {
...
func mysqlContains(slice []string, item string) bool {
	for _, s := range slice {
		if s == item {
			return true
		}
...
func mysqlCheckExistingRecord(db *sql.DB, majorValue, minorValue, version string) int {
	var id int
...
func mysqlcleardata(host, port, database, username, password string, logger *log.Logger) {
	// Establish a database connection
	db, err := sql.Open("mysql", fmt.Sprintf("%s:%s@tcp(%s:%s)/%s", username, password, host, port, database))
	if err != nil {
		logger.Fatalf("Error connecting to mysql database: %v", err)
...
func mysqlclearAllData(host, port, database, username, password string, logger *log.Logger) {
	db, err := sql.Open("mysql", fmt.Sprintf("%s:%s@tcp(%s:%s)/%s", username, password, host, port, database))
	if err != nil {
		logger.Fatalf("Error connecting to mysql database: %v", err)
	}
...
func mysqlcheckScripts(host, port, database, username, password, folderPath string, logger *log.Logger) {
	// Establish a database connection
	db, err := sql.Open("mysql", fmt.Sprintf("%s:%s@tcp(%s:%s)/%s", username, password, host, port, database))
	if err != nil {
		logger.Fatalf("Error connecting to mysql database: %v", err)
	...
	executedScripts := mysqlexecutedScriptsWithHashes(db)
	...
	err = filepath.Walk(folderPath, func(path string, info os.FileInfo, err error) error {
		...
		if !info.IsDir() && strings.HasSuffix(info.Name(), ".sql") {
			versions, fileName, hashCodes, _, _ := mysqlGetVersion(path)
	...
	mysqlprintMissingScripts(executedScripts, folderScripts, logger)
}

func mysqlexecutedScriptsWithHashes(db *sql.DB) map[string]map[string]string {
...
func mysqlprintMissingScripts(executedScripts map[string]map[string]string, folderScripts map[string]map[string]string, logger *log.Logger) {
...
func mysql_processFiles(folderPath, prescript, postscript, difference string) error {
	...
	statementsFile1 := mysql_splitSQLStatements(string(file1Content))
	statementsFile2 := mysql_splitSQLStatements(string(file2Content))

	// Find differences
	differences := mysql_findDifferences(statementsFile1, statementsFile2)
	fmt.Println("Differences have been saved to", difference)
	// Save the differences to a new SQL file
	return mysql_saveDifferencesToFile(outputPath, differences)
}

// splitSQLStatements splits the SQL script into individual statements.
func mysql_splitSQLStatements(content string) []string {
...
func mysql_findDifferences(file1Statements, file2Statements []string) []string {
...
func mysql_saveDifferencesToFile(filename string, differences []string) error {
